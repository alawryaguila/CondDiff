...
__all__ = ["DiffusionModelUNet", "DiffusionModelCondUNet"]
...
class DiffusionModelCondUNet(DiffusionModelUNet):
    """
    Unet network with timestep embedding and attention mechanisms for conditioning based on
    Rombach et al. "High-Resolution Image Synthesis with Latent Diffusion Models" https://arxiv.org/abs/2112.10752
    and Pinaya et al. "Brain Imaging Generation with Latent Diffusion Models" https://arxiv.org/abs/2209.07162
    ...
    def forward(
        ...
        x: torch.Tensor,
        t: torch.Tensor,
        cond: torch.Tensor, #TODO check if this is right
        **kwargs,
    ) -> torch.Tensor:
        """
        Args:
            x: input tensor (N, C, SpatialDims).
            x_cond: ccondition tensor (N, C, SpatialDims).
            t: timestep tensor (N,).
            y: context tensor (N, 1, ContextDim).
            class_labels: context tensor (N, ).
            down_block_additional_residuals: additional residual tensors for down blocks (N, C, FeatureMapsDims).
