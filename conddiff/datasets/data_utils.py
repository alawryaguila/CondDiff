...
def read_and_deform(file_name, dtype, deform_dict, device, mask=None, default_value_linear_mode = None, deform_mode = 'linear', mean = 0., scale = 1.):
    ...
    #print(f"file_name: {file_name} and Iimg: {Iimg.shape}")
    ...
    I = torch.nan_to_num(I) #.to(device, non_blocking=True)
...
def read_and_deform_image(exist_keys, task_name, file_name, setups, deform_dict, device, **kwargs):
   ...
   # print(f"task_name: {task_name} and Idef: {Idef.shape}")
   # exit()
...
def read_and_deform_pathology(exist_keys, task_name, file_name, setups, deform_dict, device, mask = None, 
    ...
    if kwargs['training'] == False:
        #set seed
        np.random.seed(42) #TODO check if want to do this or set it to idx
...
def augment_pathology(Pprob, pde_func, t, shape_gen_args, device, save_orig_for_visualize = False, seed = None):
    ...
    if seed is None:
        seed = int(time.time()) #TODO set this to a fixed value at test time
    np.random.seed(seed) 
...
def read_pathology(exist_keys, task_name, file_name, setups, deform_dict, device, 
    ...
    if file_name is None:
        ...
        #update_dict['pathology_orig'] = torch.zeros((x2-x1, y2-y1, z2-z1))[None]
    ...
    if file_name == 'random_shape': # generate random shape
        ...
        Pp, _, aff = read(file_name, torch.float, device, range_list = [x1, x2, y1, y2, z1, z2]) #TODO check that this is right - otherwise might need to crop to T1_shape
        #Pp, _, _ = read(file_name, torch.float, device, range_list = None)
        Pp_orig = Pp #.to(device)
    ...
    if Pb.mean() <= shape_gen_args.pathol_tol: #TODO - check what this bit does
        update_dict['pathology'] = torch.zeros((x2-x1, y2-y1, z2-z1))[None]
        update_dict['pathology_prob'] = torch.zeros((x2-x1, y2-y1, z2-z1))[None]
        return update_dict

    update_dict['pathology'] = Pb[None]
    #update_dict['pathology_orig'] = Pp_orig[None]
