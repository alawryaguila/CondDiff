...
from .constants import n_neutral_labels_brainseg_with_extracerebral, label_list_segmentation_brainseg_with_extracerebral, processing_funcs
...
class BaseGen(Dataset):
    ...
    def prepare_tasks(self):
        ...
        if ('encode_anomaly' in self.tasks or 'pathology' in self.tasks) and self.synth_args.augment_pathology: 
    ...
    def read_input(self, idx): #only 1 modality is used at a time?
        """
        ...
        return case_name, input_mode, img, aff, res, flip2orig, mniaffine, pathology_files
    ...
    def __getitem__(self, idx): #why is this generating multiple samples?
...
class UNAModule(UNAGen, pl.LightningDataModule):
    def __init__(self, args, training_=True, device='cpu'):
        super(UNAModule, self).__init__(args.data_config_path, training_, device=device)
    ...
    def setup(self, stage=None):
        if self.args.training_:
            ...
            self.train_dataset = UNAGen(self.args.data_config_path, training_=self.args.training_, device=self.device)
            ...
            self.val_dataset = UNAGen(val_data_config_path, training_=self.val_args.training_, device=self.device)
        else:
            ...
            self.train_dataset = UNAGen(test_data_config_path, training_=self.test_args.training_, device=self.device)
